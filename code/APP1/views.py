...
def fun(request):
    ...
    word = json_dict.get("key")
    num = int(json_dict.get("num"))
    startTime = json_dict.get("startTime")
    endTime = json_dict.get("finishTime")
    startTime = startTime.split("T", 1)[0]
    endTime = endTime.split("T", 1)[0]

    # 创建一个子文件夹用于存放下载数据
    Rep(startTime,endTime,num,word)
    print("文件保存成功！")
    pythoncom.CoUninitialize()
    return JsonResponse({"path": 'D:\\数据科学爬取案件'})
...
def download(startTime,endTime,num,word):
    # 移动到googlechrome并点击
